...
try:
    ...
    xt_available = True
    logger.info("[OK] QMT (xtquant.xtdata) imported successfully")
    # ============================================================
    ...
    if not _patch_already_applied:
        ...
        logger.info("[OK] xtdata download functions patched with global thread lock (RLock)")
    else:
        logger.info("[OK] xtdata already patched, reusing existing lock")
except ImportError:
...
if os.environ.get('XQSHARE_REMOTE_HOST'):
    try:
        ...
        if xqshare_client.is_connected():
            ...
            logger.info("[OK] xqshare (远程xtquant) 连接成功")
        else:
            logger.info("[INFO] xqshare 连接失败")
    except ImportError:
        logger.info("[INFO] xqshare 未安装")
...
try:
    ...
    TDX_AVAILABLE = True
    logger.info("[OK] TDX (通达信) provider available")
except ImportError:
    logger.info("[INFO] TDX provider not available")
...
try:
    ...
    logger.info("[OK] Eastmoney (东方财富) provider available")
except ImportError:
    logger.info("[INFO] Eastmoney provider not available")
# 打印数据源状态总结
if not (xt_available or TDX_AVAILABLE or EASTMONEY_AVAILABLE):
    logger.warning("\n[WARN] 所有数据源都不可用，请至少安装一个：")
    logger.info("  - QMT (推荐用于实盘)")
    logger.info("  - 或使用 TDX/Eastmoney (可用于回测)")
...
class DuckDBDataReader:
    ...
    def _connect(self):
        ...
        try:
            ...
            logger.error("[ERROR] duckdb未安装，请运行: pip install duckdb")
            raise
        except Exception as e:
            logger.error(f"[ERROR] DuckDB连接失败: {e}")
            raise
    ...
    def _check_tables(self):
        ...
        try:
            ...
            if count == 0:
                logger.warning(f"[WARN] stock_daily 表存在但数据为空（0 条记录），请先下载日线数据")
            else:
                date_range = self.conn.execute(
                    "SELECT MIN(date), MAX(date), COUNT(DISTINCT stock_code) FROM stock_daily"
                ).fetchone()
                logger.info(f"[OK] stock_daily: {date_range[2]} 只股票, {date_range[0]} ~ {date_range[1]}")
        except Exception as e:
            logger.warning(f"[WARN] 检查数据表失败: {e}")
    ...
    def get_market_data(self, stock_list: List[str], start_date: str, end_date: Optional[str] = None) -> pd.DataFrame:
        ...
        try:
            ...
            if 'stock_daily' in err_msg and 'does not exist' in err_msg:
                logger.error(f"[ERROR] 数据查询失败: stock_daily 表不存在")
                logger.info("        请先下载日线数据: python run_gui.py → Tushare下载 → 勾选「日线行情」")
            else:
                logger.error(f"[ERROR] 数据查询失败: {e}")
            return pd.DataFrame()
    ...
    def get_stock_info(self, stock_code: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"[ERROR] 查询股票信息失败: {e}")
            return None
...
class DataAPI:
    ...
    def connect(self) -> bool:
        ...
        # 优先级1: 尝试连接 QMT (本地)
        if xt_available:
            logger.info("  Trying QMT (xtquant)...")
            if self._connect_qmt():
                self._active_source = 'qmt'
                logger.info("[OK] Using QMT (xtquant) as data source")
                # ✓ 不要直接返回，继续初始化备用数据源
            else:
                logger.warning("  [WARN] QMT connection failed")
        # 优先级2: 降级到 xqshare (远程)
        if self._active_source is None and XQSHARE_AVAILABLE:
            logger.info("  Trying xqshare (远程xtquant)...")
            if self._connect_xqshare():
                self._active_source = 'xqshare'
                logger.info("[OK] Using xqshare (远程xtquant) as data source")
            else:
                logger.warning("  [WARN] xqshare connection failed")
        # 优先级3: 降级到 TDX
        if self._active_source is None and TDX_AVAILABLE:
            logger.info("  Trying TDX (通达信)...")
            if self._connect_tdx():
                self._active_source = 'tdx'
                logger.info("[OK] Using TDX (通达信) as data source")
            else:
                logger.warning("  [WARN] TDX connection failed")
        # 优先级4: 降级到 Eastmoney
        if self._active_source is None and EASTMONEY_AVAILABLE:
            logger.info("  Trying Eastmoney (东方财富)...")
            if self._connect_eastmoney():
                self._active_source = 'eastmoney'
                logger.info("[OK] Using Eastmoney (东方财富) as data source")
            else:
                logger.warning("  [WARN] Eastmoney connection failed")
        ...
        if self._active_source in ('qmt', 'xqshare'):
            ...
            if TDX_AVAILABLE and self._tdx_provider is None:
                logger.info("  Initializing TDX provider (for fallback)...")
                try:
                    self._connect_tdx()
                    logger.info("  [OK] TDX provider ready")
                except Exception as e:
                    logger.info(f"  [FAIL] TDX provider failed: {e}")
            if EASTMONEY_AVAILABLE and self._eastmoney_provider is None:
                logger.info("  Initializing Eastmoney provider (for fallback)...")
                try:
                    self._connect_eastmoney()
                    logger.info("  [OK] Eastmoney provider ready")
                except Exception as e:
                    logger.info(f"  [FAIL] Eastmoney provider failed: {e}")
        # 检查是否至少有一个数据源连接成功
        if self._active_source is None:
            logger.error("[ERROR] All data sources failed to connect")
            return False
    ...
    def _connect_qmt(self) -> bool:
        ...
        if not self.xt:
            ...
            if not getattr(self, '_qmt_recovery_attempted', False):
                ...
                logger.info("[INFO] xtdata 不可用，尝试自动启动 QMT...")
                try:
                    from core.auto_login import QMTAutoLogin
                    if QMTAutoLogin().login(restart=False, timeout=60):
                        import xtquant.xtdata as xtdata
                        self.xt = xtdata
                        logger.info("[OK] QMT 启动成功，xtdata 已加载")
                except Exception as e:
                    logger.warning(f"[WARN] QMT 启动失败: {e}")
            if not self.xt:
                return False
    ...
    def _connect_xqshare(self) -> bool:
        ...
        try:
            ...
            logger.info(f"  [INFO] xqshare connection failed: {e}")
        return False
    ...
    def _connect_tdx(self) -> bool:
        ...
        try:
            ...
            logger.info(f"  [INFO] TDX connection failed: {e}")
            return False
    ...
    def _connect_eastmoney(self) -> bool:
        ...
        try:
            ...
            logger.info(f"  [INFO] Eastmoney connection failed: {e}")
            return False
            ...
            if period == 'tick':
                    ...
                    if tick_data is not None and len(tick_data) > 0:
                        ...
                        if time_field:
                            ...
                            try:
                                ...
                                logger.info(f"时间字段转换失败: {e}")
                                # 使用当前时间作为默认值
                                df['time'] = pd.Timestamp.now()
                        else:
                            # 如果没有找到时间字段，使用当前时间
                            logger.info("警告: 未找到时间字段，使用当前时间")
                            df['time'] = pd.Timestamp.now()
                ...
                if result_list:
                    ...
                    try:
                        ...
                        if notna_count == 0:
                            logger.info("警告: 时间格式转换失败")
                    except Exception as e:
                        logger.info(f"时间格式处理警告: {e}")
    ...
    def _get_price_tdx(self, codes, start, end, period, count, fields, adjust):
        ...
        if count:
            ...
            if start_date and end_date:
                try:
                    ...
                    logger.info(f"[INFO] TDX不支持指定日期范围，已自动转换为获取最近{count}条数据")
        ...
        if period == '1m' and count is not None and count < 2:
            ...
            if original_count != count:
                logger.info(f"[INFO] TDX 1分钟数据最小count为2，已自动调整: {original_count} -> {count}")
        try:
            # 批量获取K线数据
            all_data = []
            for code in codes:
                ...
                for stock in stock_list:
                    try:
                        ...
                        try:
                            ...
                            if stock in test_data and test_data[stock] is not None and len(test_data[stock]) > 0:
                                results[stock] = True
                                logger.info(f"{stock} 历史数据下载完成并验证成功")
                            else:
                                results[stock] = False
                                logger.info(f"{stock} 历史数据下载完成但验证失败")
                        except Exception:
                            results[stock] = False
                            logger.info(f"{stock} 历史数据下载完成但验证失败")
                    except Exception as e2:
                        results[stock] = False
                        logger.info(f"{stock} 下载失败: {e2}")
        return results
    ...
    def get_price_robust(self,
        ...
        for attempt in range(max_retries):
            try:
                ...
                if period == 'tick':
                    ...
                    if not has_data:
                        ...
                        raise DataError(
                            ...
                            f"🔧 快速解决方案（推荐）：\\n"
                            f"   python tools/download_all_stocks.py\\n\\n"
                            f"或使用GUI下载：python run_gui.py → 数据管理 → 下载股票数据\\n\\n"
                            f"或安装免费在线数据源（无需QMT）：pip install pytdx\\n\\n"
                            f"详细说明：docs/assets/TROUBLESHOOTING.md 第3节"
                        )
                ...
                if attempt < max_retries - 1:
                    logger.info(f"第{attempt+1}次尝试失败: {str(e)}")
                    logger.info("等待3秒后重试...")
                    time.sleep(3)
                else:
                    break
        ...
        logger.info(f"[降级] QMT 获取失败（{max_retries}次重试），尝试备用数据源...")
        ...
        if fallback_df is not None and not fallback_df.empty:
            ...
            logger.info(f"[降级] 成功从备用数据源获取数据 (来源: {source})")
            return fallback_df
        ...
        for code in codes:
            try:
                ...
                if first_seq_id == -1 and seq_id > 0:
                    first_seq_id = seq_id
                logger.info(f"  {code} 订阅成功，订阅号: {seq_id}")
            except Exception as e:
                logger.info(f"  {code} 订阅失败: {e}")
        return first_seq_id
    ...
    def unsubscribe_quote(self, seq_id: int) -> bool:
        ...
        try:
            ...
            logger.info(f"  订阅号 {seq_id} 已取消订阅")
            return True
        except Exception as e:
    ...
    def run_forever(self, check_interval: float = 1.0):
        ...
        logger.info("✓ 开始接收行情推送，按 Ctrl+C 退出...")
        try:
            ...
            if hasattr(self.xt, 'run'):
                ...
                import time
                logger.info(f"  使用兼容模式，检查间隔: {check_interval}秒")
                while True:
